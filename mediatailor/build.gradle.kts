...
val publishIntegration by tasks.registering {
    group = "publishing"
    description = "Publishes the MediaTailor integration library to Artifactory"

    val clean by tasks
    val assembleRelease by tasks
    val artifactoryPublish by tasks

    artifactoryPublish.shouldRunAfter(assembleRelease)
    assembleRelease.shouldRunAfter(clean)

    dependsOn(clean, assembleRelease, artifactoryPublish)
}
