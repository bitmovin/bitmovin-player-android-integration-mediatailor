...
class MediaTailorSessionManagerSpec : DescribeSpec({
    ...
    beforeEach {
        ...
        dependencyFactory = mockk<DependencyFactory> {
            ...
            every { createAdBeaconing(any(), any(), any(), any(), any()) } returns adBeaconing
            every { createScope(any()) } returns scope
        }
        mediaTailorSessionManager = InternalMediaTailorSessionManager(
            mockk(relaxed = true),
            dependencyFactory,
        )
    }
        ...
        describe("when initialization is successful") {
            ...
            it("creates dependencies") {
                ...
                verify {
                    ...
                    dependencyFactory.createAdBeaconing(any(), any(), any(), any(), any())
                }
            }
        }
        ...
        describe("when initialization fails") {
            ...
            describe("sending a tracking event") {
                it("emits an error event") {
                    ...
                    coVerify { eventEmitter.emit(capture(slot)) }
    ...
    describe("destroying a media tailor session manager") {
        beforeEach {
            coEvery { mediaTailorSession.initialize(any()) } returns Success("")
            mediaTailorSessionManager.initializeSession(
                sessionConfig = MediaTailorSessionConfig(
        ...
        it("stops the session and disposes all dependencies") {
        ...
        it("cancels the session manager scope") {
            mediaTailorSessionManager.destroy()

            expectThat(scope.isActive).isFalse()
        }
    }
})
